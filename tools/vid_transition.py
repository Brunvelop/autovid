...
class DataHandler:
    ...
    def merge_video_chunks2(self):  # TODO implement
    ...
    def merge_video_chunks(self):
        ...
        # Load the merged video into a MoviePy clip and return it
        merged_clip = VideoFileClip(str(self.merged_vid))
        return merged_clip
...
def main(input_videos=INPUT_VIDEOS, num_frames=NUM_FRAMES, animation=ANIMATION, output=OUTPUT, max_rotation=MAX_ROTATION, 
          max_distortion=MAX_DISTORTION, max_blur=MAX_BLUR, max_brightness=MAX_BRIGHTNESS, max_zoom=MAX_ZOOM, 
          debug=DEBUG, art=ART, remove=REMOVE_ORIGINAL, merge=MERGE_PHASES):

    with tempfile.TemporaryDirectory() as tmp_dir:
        tmp_path = pathlib.Path(tmp_dir)
        dh = DataHandler()
        args = argparse.Namespace(input=input_videos, num_frames=num_frames, animation=animation, output=output, 
                                  max_rotation=max_rotation, max_distortion=max_distortion, max_blur=max_blur, 
                                  max_brightness=max_brightness, max_zoom=max_zoom, debug=debug, art=art, 
                                  remove=remove, merge=merge)
        if not dh.verify_arguments(args, tmp_path):
            return

        actions_determinator = AnimationActions(max_zoom, max_brightness, max_rotation, max_blur,
        ...
        final_phase_folder = AnimationImages.make_transition(dh.tmp_path, dh.phase1_images, dh.phase2_images,
                                                             phase1_actions, phase2_actions, debug)
